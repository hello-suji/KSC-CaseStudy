...
| 평가 항목      | pytest | 본 연구 복합 목표 미적용 | 본 연구 복합 목표 적용 K=8 | 본 연구 복합 목표 적용 K=5 |
| ---------- | ------ | -------------- | ----------------- | ----------------- |
| 실행된 테스트 개수 | 3      | 3              | 9                 | 5                 |
| 라인 커버리지(%) | 33.7%  | 46.03%         | 66.22%            | 50.66%            |
| 분기 커버리지(%) | 36.36% | 61.36%         | 87.72%            | 63.64%            |

### 결과 해석

기준선 pytest는 단 3개의 테스트만 실행되어 라인 33.7%, 분기 36.36%로 낮은 커버리지를 보였다. 미적용군에서는 테스트를 새로 생성하지 않았음에도 실행 과정에서 더미 객체 패치가 적용되면서 기존 테스트가 정상적으로 끝까지 수행되었고, 그 결과 라인·분기 커버리지가 소폭 증가하였다. 반면 복합 목표 적용군은 분기 흐름·정의–사용 체인·예외 흐름을 통합해 경로 타격 범위를 확장하여 가장 높은 커버리지를 달성하였다. 특히 K=8 설정에서는 라인 66.22%, 분기 87.72%로 가장 큰 향상을 보였다.
